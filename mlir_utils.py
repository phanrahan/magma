...
def _make_hw_array_get_op(ctx: MlirContext, inst: m.Circuit) -> MlirMultiOp:
    ...
    value = ctx.named_value(MlirIntegerType(width))
...
def _make_mux_op(ctx: MlirContext, inst: m.Circuit) -> MlirMultiOp:
    ...
    data_value = ctx.named_value(magma_type_to_mlir_type(type(defn.I.data)))
    sel_value = ctx.named_value(magma_type_to_mlir_type(type(defn.I.sel)))
    getter = HwArrayGetOp(inst.name)
    g.add_nodes_from(
...
def _make_reg_op(ctx: MlirContext, inst: m.Circuit) -> MlirMultiOp:
    ...
    reg_init_value = ctx.named_value(value_type)
    g.add_nodes_from((
