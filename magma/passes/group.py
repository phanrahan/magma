...
class _GrouperBase(abc.ABC):
    ...
    def _is_external_drivee(self, value):
        ...
        if is_external is not None:
            return is_external
        # Check externality of all driving values (recursively)
        results = [self._is_external_drivee(drivee)
                   for drivee in value.driving()]
        if len(results) == 0:
            return None

        # NOTE: For now, we only support anon values that are driving values
        # with the same status: either all must be external or internal. We
        # also assume that more drivees will not be added after the fact. We
        # still do not support unwired anon values, nor do we support anon
        # values that have wirings that change after the compile_guard (e.g. an
        # internal value is then wired to an external value after the compile
        # guard)

        # In the future, we could improve support by:
        # (1) staging the resolving of externality until the end of definition
        # (2) copy anon values that drive mixed values (one copy for external,
        #     one for internal)
        # (3) do final wiring for internal/external copies with the proper
        #     values
