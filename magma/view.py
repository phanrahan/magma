...
class PortView(MagmaProtocol, metaclass=PortViewMeta):
    ...
    def __init__(self, port, view_parent):
        self.port = port
    ...
    def __getitem__(self, key):
        ...
        return PortView[type(item)](item, self.view_parent)
    ...
    def __getattr__(self, key):
        ...
        return PortView[type(attr)](attr, self.view_parent)

    def path(self) -> Tuple[str]:
        path = (self.port.name.name,)
        curr = self.view_parent
    ...
    def get_hierarchical_coreir_select(self):
        if self.view_parent is None:
            return ";"
        return ";".join(self.view_parent.path()) + ";"
    ...
    def __next__(self):
        n = next(self.__iter)
        return PortView[type(n)](n, self.view_parent)


class InstView:
    def __init__(self, inst, view_parent=None):
        self.inst = inst
    ...
    def path(self) -> Tuple[str]:
        ...
        if self.view_parent is None:
            return path
        return self.view_parent.path() + path
