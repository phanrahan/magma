...
def test_module():
    ...
    ostream.seek(0)
    version_string = ostream.read()
    pattern = (
        r"// Generated by CIRCT firtool-(?P<major_version>\d+)"
        r".(?P<minor_version>\d+).(?:\d+)(?:-\d+-[0-9a-f]+)?$"
    )
    match = re.match(pattern, version_string)
    assert match
    assert int(match.group("major_version")) == 1
    assert int(match.group("minor_version")) >= 51
