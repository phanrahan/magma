...
def test_unconnected_io(caplog):
    with magma_debug_section():
        ...
        expected = """\x1b[1mtests/test_deprecated/test_old_io_syntax/test_unconnected.py:8\x1b[0m: Output port _Circuit.O not driven
>>     class _Circuit(m.Circuit):"""
...
def test_unconnected_instance(caplog):
    with magma_debug_section():
        ...
        expected = """\x1b[1mtests/test_deprecated/test_old_io_syntax/test_unconnected.py:32\x1b[0m: Input port buf.I not driven
>>             buf = _Buffer()"""
        assert has_error(caplog, expected)
