...
# TODO: This needs to be polymorphic, assume uint for now
    ...
    def adc(a: Data, b: Data, c: Bit) -> (Data, Bit):
        a = m.uint(m.zext(a, 1))
        b = m.uint(m.zext(b, 1))
        c = m.uint(m.zext(m.bits(c, 1), DATAWIDTH))
        res = a + b + c
    ...
    class PE:
        ...
        def __call__(self, inst: Inst, data0: Data, data1: Data = Data(0),
                     bit0: Bits[1] = Bit(0), bit1: Bits[1] = Bit(0), bit2:
                     Bits[1] = Bit(0), clk_en: Bit = Bit(1)) -> (Data, Bit,
                                                                 Bit):
