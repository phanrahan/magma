module attributes {circt.loweringOptions = "locationInfoStyle=none"} {
    hw.module @test_when_lazy_array_slice_overlap(%I: i4, %S: i1) -> (O: i4) {
        ...
        sv.verbatim "WHEN_ASSERT_123: assert property (({{0}}) |-> ({{1}} == {{2}}));" (%S, %10, %16) : i1, i2, i2
        %17 = comb.extract %I from 2 : (i4) -> i1
        sv.verbatim "WHEN_ASSERT_124: assert property (({{0}}) |-> ({{1}} == {{2}}));" (%S, %11, %17) : i1, i1, i1
        %18 = comb.extract %I from 3 : (i4) -> i1
        sv.verbatim "WHEN_ASSERT_125: assert property (({{0}}) |-> ({{1}} == {{2}}));" (%S, %12, %18) : i1, i1, i1
        %20 = hw.constant -1 : i1
        %19 = comb.xor %20, %S : i1
        %21 = comb.concat %18, %17 : i1, i1
        sv.verbatim "WHEN_ASSERT_126: assert property (({{0}}) |-> ({{1}} == {{2}}));" (%19, %10, %21) : i1, i2, i2
        sv.verbatim "WHEN_ASSERT_127: assert property (({{0}}) |-> ({{1}} == {{2}}));" (%19, %11, %14) : i1, i1, i1
        sv.verbatim "WHEN_ASSERT_128: assert property (({{0}}) |-> ({{1}} == {{2}}));" (%19, %12, %15) : i1, i1, i1
        hw.output %13 : i4
    }
}
