module attributes {circt.loweringOptions = "locationInfoStyle=none"} {
    hw.module @test_when_2d_array_assign_slice(%I: !hw.array<4xi2>, %S: i1) -> (O: !hw.array<4xi2>) {
        %1 = sv.reg name "_WHEN_WIRE_131" : !hw.inout<!hw.array<4xi2>>
        ...
        sv.verbatim "WHEN_ASSERT_270: assert property (({{0}}) |-> ({{{1}}, {{2}}} == {{{3}}, {{4}}}));" (%S, %12, %13, %14, %15) : i1, i2, i2, i2, i2
        %16 = hw.array_get %0[%2] : !hw.array<4xi2>, i2
        %17 = hw.array_get %I[%2] : !hw.array<4xi2>, i2
        sv.verbatim "WHEN_ASSERT_271: assert property (({{0}}) |-> ({{1}} == {{2}}));" (%S, %16, %17) : i1, i2, i2
        %18 = hw.array_get %0[%4] : !hw.array<4xi2>, i2
        %19 = hw.array_get %I[%4] : !hw.array<4xi2>, i2
        sv.verbatim "WHEN_ASSERT_272: assert property (({{0}}) |-> ({{1}} == {{2}}));" (%S, %18, %19) : i1, i2, i2
        %21 = hw.constant -1 : i1
        %20 = comb.xor %21, %S : i1
        sv.verbatim "WHEN_ASSERT_273: assert property (({{0}}) |-> ({{1}} == {{2}}));" (%20, %16, %15) : i1, i2, i2
        sv.verbatim "WHEN_ASSERT_274: assert property (({{0}}) |-> ({{1}} == {{2}}));" (%20, %18, %14) : i1, i2, i2
        sv.verbatim "WHEN_ASSERT_275: assert property (({{0}}) |-> ({{1}} == {{2}}));" (%20, %13, %17) : i1, i2, i2
        sv.verbatim "WHEN_ASSERT_276: assert property (({{0}}) |-> ({{1}} == {{2}}));" (%20, %12, %19) : i1, i2, i2
