...
def simulator_nested(simple):
    ...
    for i in range(inDims[0]):
        for j in range(inDims[1]):
            for k in range(inDims(2)):
                sim.set_value(testcircuit.I[i][j][k],
                              int2seq((((i*inDims[1])+j)*inDims[2])+k, width), scope)
    sim.evaluate()
    sim.get_value(testcircuit.I)
    sim.get_value(testcircuit.O)
